...
wots_pkGen input
        ...
        = _TˡStream @alg  @(Len alg) @dom (zip3C (pkSeed input) wotspkADRS² (tmp alg))
            where 
                ...
                tmp ∷ (KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg) ⇒ Proxy alg → Channel dom (Vec (Len alg * N alg) (ByteType))
                tmp alg 
...
-- -- Algorithm 8 
-- wots_pkFromSig ∷ ∀ alg dom . (KnownDomain dom, HiddenClockResetEnable dom, KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg (Len alg)) 
...
--                             = (++) <$> msg⁰ <*> (fmap (unconcatBitVector# . resize) csum)
--                         -- Line 8 - 11
--                         tmp ∷ (HiddenClockResetEnable dom, KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg 0)
--                             ⇒  Channel dom (Vec (Len alg * N alg) (ByteType))
--                         tmp 
--                             | SLH_DSAParametersFacts {} ← knownSLH_DSAParameters @alg
--                             = fmap concat (concatMapC (map go  (iterateI @(Len alg) (+1) (natToNum @0))))
--                             where
--                                 go ∷ (HiddenClockResetEnable dom, KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg 0) 
--                                     ⇒ IdxType alg → Channel dom (NBlockType alg)
--                                 go i 
--                                  | SLH_DSAParametersFacts {} ← knownSLH_DSAParameters @alg
--                                  = chain² @(2^Lgʷ alg) (zip3C (sigⁱ i) pkSeed adrs) (fmap (\ x → resize (x !! i)) msg¹)  (fmap (\x → (natToNum @(W alg - 1)) - (resize (x !! i))) msg¹)  
--                                     where
--                                         sigⁱ ∷ (HiddenClockResetEnable dom, KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg 0) ⇒ IdxType alg → Channel dom (NBlockType alg)
--                                         sigⁱ i⁰
--                                             | SLH_DSAParametersFacts {} ← knownSLH_DSAParameters @alg
--                                             = fmap (\x → x !! i⁰) sig 
--                         wotspkADRS ∷ Channel dom (ADRSType alg)
...
-- -- Algorithm 9
-- xmss_node ∷ ∀ (alg ∷ SLH_DSA)  dom . (KnownDomain dom, HiddenClockResetEnable dom,  KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg 0) 
...
-- -- Algorithm 10
-- xmss_sign ∷ ∀ (alg ∷ SLH_DSA)  dom . (KnownDomain dom, HiddenClockResetEnable dom,  KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg 0) 
...
--     where 
--         object ∷  ( KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg i)
...
-- -- -- Algorithm 11
-- xmss_pkFromSig ∷ ∀ (alg ∷ SLH_DSA)  dom ℓ. (KnownDomain dom, HiddenClockResetEnable dom,  KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg ℓ, KnownNat ℓ) 
...
--         -- line 5
--         node⁰ ∷ (SLH_DSA_hashStreamFact alg (Len alg)) ⇒ Channel dom (PKˢⁱᵍType alg)
--         node⁰  
--             | SLH_DSAParametersFacts {} ← knownSLH_DSAParameters @alg
...
-- -- Algorithm 12
-- ht_sign ∷ ∀ (alg ∷ SLH_DSA)  dom . (KnownDomain dom, HiddenClockResetEnable dom,  KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg 0) 
...
-- -- Algorithm 13
-- ht_verify ∷ ∀ (alg ∷ SLH_DSA)  dom ℓ . (KnownDomain dom, HiddenClockResetEnable dom,  KnownSLH_DSAParameters alg, SLH_DSA_hashStreamFact alg ℓ, KnownNat ℓ) 
