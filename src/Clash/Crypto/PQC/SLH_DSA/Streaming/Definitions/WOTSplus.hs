...
chain ∷ ∀ i s (alg ∷ SLH_DSA) dom . (KnownDomain dom, HiddenClockResetEnable dom,  KnownSLH_DSAParameters alg,
         KnownNat i, KnownNat s, 0 ≤ i, SLH_DSA_hashStreamFact alg 0) 
...
--                     → IdxType alg
--                     → Channel dom (NBlockType alg, PKSeedType alg, ADRSType alg)
--                 function alg x1 j = zip3C (_FStream (fmap (go j) x1)) pkSeed adrs
--                     where 
--                         pkSeed ∷ Channel dom (PKSeedType alg)
--                         pkSeed = sndOf3C x1
--                         adrs ∷ Channel dom (ADRSType alg)
...
--                     → Channel dom (IdxType alg)
--                     → Channel dom (NBlockType alg, PKSeedType alg, ADRSType alg)
--                 function alg x1 j = zip3C (_FStream go) pkSeed adrs
--                     where 
--                         pkSeed ∷ Channel dom (PKSeedType alg)
--                         pkSeed = sndOf3C x1
--                         adrs ∷ Channel dom (ADRSType alg)
...
--         = _TˡStream @dom @(Len alg) alg (zip3C (pkSeed input) wotspkADRS² (tmp alg))
--             where 
